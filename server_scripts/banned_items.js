...
PlayerEvents.inventoryChanged(event =>{
  ...
  if(!player) return;
  let foundBannedItem = BANNED_ITEMS.find(bannedItem => bannedItem.id === item.id);
  ...
  event.server.tell(`§4${player.name.getString()}, you are not allowed to have §f${foundBannedItem.display}§4!`);
})

ServerEvents.commandRegistry(event => {
  const { commands: Commands, arguments: Arguments } = event

  event.register(Commands.literal('banneditems')
    .executes(c => {
      /**
       * @type {$Player}
       */

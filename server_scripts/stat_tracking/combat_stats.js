...
ServerEvents.commandRegistry(event => {
  ...
  event.register(Commands.literal('kills')
    ...
    .then(Commands.argument('target', Arguments.PLAYER.create(event))
      .executes(c => kills(c.source.player, Arguments.PLAYER.getResult(c, 'target')))
    )
  )

  event.register(Commands.literal('deaths')
    .executes(c => deaths(c.source.player, c.source.player))
    .then(Commands.argument('target', Arguments.PLAYER.create(event))
      .executes(c => deaths(c.source.player, Arguments.PLAYER.getResult(c, 'target')))
    )
  )
...
function getCombatStats(player) {
  try {
    let cs = [
      `§5☻ ${player.name}§f: §a${player.kills}§f/§4${player.player_deaths} §f(§a${parseFloat(player.kdr).toFixed(2)}§f KDR)`,
      ``,
      `§7◆ Killstreak: §a${player.killstreak} §e✦${player.highest_killstreak}`,
      `§7◆ Nemesis: ${getNemesis(player)}`,
      `§7◆ Victim: ${getVictim(player)}`,
      `§7◆ Favourite Weapon: §a${getFavouriteWeapon(player)}`
    ];
    let favSpell = getFavouriteSpell(player.kill_history);
    if (favSpell) {
      if (favSpell.spell.name) {
        cs.push(`§7◆ Favourite Spell: §a${favSpell.spell.name} §f(§a${favSpell.count}§f)`);
        cs.push(`§7◆ Glyphs: §a${favSpell.spell.glyphs}`);
      } else {
    ...
    print(e);
    return ["§cAn error occurred while fetching stats"];
  }
}
...
function getNemesis(player) {
  ...
  let nemesis = leaderboard.reduce((acc, p) => {
    if (!p || p.uuid === player.uuid || !p.kill_history || p.kill_history.length == 0) return acc;
    let kills = p.kill_history.filter(k => k && k.uuid === player.uuid).length;
    if (kills > acc.kills) {
...
let KDAMenu = new Menu({
  ...
  load: function (menu, data) {
    ...
    let leaderboard = getLeaderboard().sort(COMBAT_LEADERBOARD_SORTING_OPTIONS[sortInd].sort);
    ...
    leaderboard.slice(0, 4 * MAX_COLUMNS).forEach((statPlayer, i) => {
      ...
      menu.gui.slot(column, row, slot => {
        slot.item = createMenuButton({
          ...
          description: getCombatStats(statPlayer),
          itemID: "minecraft:player_head",
          components: { profile: statPlayer.name },
        });
        slot.leftClicked = () => menu.ShowPage("personal", { target: statPlayer, sort: sortInd });
      });
    });
  ...
  load: function (menu, data) {
    ...
    menu.gui.slot(0, 0, slot => {
      slot.item = createMenuButton({
        ...
        slot.leftClicked = e => {
          menu.ShowPage("killHistory", { target: statPlayer, sort: data.sort, viewType: "heads" });
        }
    })
    menu.gui.slot(1, 0, slot => {
      slot.item = createMenuButton({
        ...
        slot.leftClicked = e => {
          menu.ShowPage("deathHistory", { target: statPlayer, sort: data.sort, viewType: "heads" });
        }
    })
  }
},
  ...
  load: function (menu, data) {
    ...
    if (killHistory && killHistory.length != 0) {
      for (let i = 0; i < killHistory.length; i++) {
        ...
        menu.gui.slot(column, row, slot => {
          slot.item = viewType == "heads" ? getKillIcon(kill) : Item.of(`${kill.weapon.id}${kill.weapon.components ? `${kill.weapon.components}` : ""}`);
        })
      }
    }
    ...
    menu.gui.slot(0, 4, slot => {
      slot.item = createMenuButton({
        ...
        slot.leftClicked = e => {
          menu.ShowPage("personal", { target: statPlayer, sort: data.sort });
        }
    })
    ...
    menu.gui.slot(3, 4, slot => {
      slot.item = createMenuButton({
        title: `§aPrevious Page (${currentPage + 1}/${pages == 0 ? 1 : pages})`,
        itemID: "minecraft:arrow",
    ...
    menu.gui.slot(5, 4, slot => {
      slot.item = createMenuButton({
        ...
        slot.leftClicked = e => {
          ...
          menu.ShowPage("deathHistory", { target: statPlayer, sort: data.sort, viewType: viewType, page: currentPage });
        }
    })
